...
def test_signup_creates_user_and_returns_token(client):
    r = client.post("/auth/signup", json={"email": "chandra@example.com", "password": "hunter22"})
...
def test_signup_normalizes_email_case(client):
    client.post("/auth/signup", json={"email": "Chandra@Example.com", "password": "hunter22"})
    r = client.post("/auth/login", json={"email": "chandra@example.com", "password": "hunter22"})
    assert r.status_code == 200


def test_signup_rejects_duplicate_email(client):
    client.post("/auth/signup", json={"email": "dup@example.com", "password": "hunter22"})
    r = client.post("/auth/signup", json={"email": "dup@example.com", "password": "different99"})
    assert r.status_code == 400


def test_signup_rejects_weak_password(client):
    r = client.post("/auth/signup", json={"email": "weak@example.com", "password": "short"})
    assert r.status_code == 422
    r2 = client.post("/auth/signup", json={"email": "weak2@example.com", "password": "nodigitshere"})
...
def test_login_fails_with_wrong_password(client):
    client.post("/auth/signup", json={"email": "login2@example.com", "password": "hunter22"})
...
def test_login_and_signup_errors_are_identically_worded(client):
    ...
    """
    client.post("/auth/signup", json={"email": "real@example.com", "password": "hunter22"})
...
def test_password_is_never_returned_in_any_response(client):
    signup = client.post("/auth/signup", json={"email": "secret@example.com", "password": "hunter22"})
