async def test_password_redacted_when_another_field_fails_validation(client):
    ...
    """
    r = await client.post("/api/v1/auth/signup", json={"email": "redact@example.com", "password": "hunter2222"})
...
async def test_password_redacted_when_password_itself_is_invalid(client):
    r = await client.post("/api/v1/auth/signup", json={"email": "weakpw@example.com", "password": "short", "display_name": "X"})
    assert r.status_code == 422
    assert "short" not in r.text


async def test_login_is_rate_limited(client):
    await client.post("/api/v1/auth/signup", json={"email": "ratelimit@example.com", "password": "hunter2222", "display_name": "RL"})
    ...
    for _ in range(5):
        r = await client.post("/api/v1/auth/login", json={"email": "ratelimit@example.com", "password": "wrongpass1"})
...
async def test_signup_is_rate_limited(client):
    ...
    for i in range(10):
        await client.post("/api/v1/auth/signup", json={"email": f"burst{i}@example.com", "password": "hunter2222", "display_name": "X"})
    r = await client.post("/api/v1/auth/signup", json={"email": "burst-overflow@example.com", "password": "hunter2222", "display_name": "X"})
    assert r.status_code == 429
