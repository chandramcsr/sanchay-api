...
async def _signup(client, email="activity@example.com", password="hunter2222"):
    return await client.post("/auth/signup", json={"email": email, "password": password, "display_name": "Activity Test"})


async def test_successful_login_creates_a_login_event(client, db_session):
    await _signup(client)
    await client.post("/auth/login", json={"email": "activity@example.com", "password": "hunter2222"})
...
async def test_failed_login_creates_a_login_event_too(client, db_session):
    await _signup(client)
    await client.post("/auth/login", json={"email": "activity@example.com", "password": "wrongpassword1"})
...
async def test_failed_login_against_nonexistent_email_is_still_logged(client, db_session):
    ...
    await client.post("/auth/login", json={"email": "nobody-here@example.com", "password": "whatever123"})
...
async def test_login_history_requires_authentication(client):
    r = await client.get("/auth/login-history")
...
async def test_login_history_returns_own_events_newest_first(client):
    signup = await _signup(client)
    token = signup.json()["access_token"]
    await client.post("/auth/login", json={"email": "activity@example.com", "password": "wrongpass1"})
    await client.post("/auth/login", json={"email": "activity@example.com", "password": "hunter2222"})

    r = await client.get("/auth/login-history", headers={"Authorization": f"Bearer {token}"})
...
async def test_login_history_does_not_leak_other_users_events(client):
    ...
    await client.post("/auth/login", json={"email": "userA@example.com", "password": "wrongpass1"})

    r = await client.get("/auth/login-history", headers={"Authorization": f"Bearer {tokenB}"})
