...
async def test_preview_returns_group_and_inviter_name_with_no_auth(client):
...
async def test_preview_requires_no_authorization_header_at_all(client):
...
async def test_preview_404s_for_a_made_up_id(client):
    r = await client.get("/api/v1/shared-expenses/invites/00000000-0000-0000-0000-000000000000")
    assert r.status_code == 404


async def test_preview_only_exposes_group_name_inviter_and_invitee_name(client):
...
async def test_preview_returns_the_invitee_name_for_signup_prefill(client):
...
async def test_accept_adds_the_authenticated_user_to_the_group(client):
...
async def test_accept_works_even_when_joiner_email_does_not_match_the_invited_email(client):
    """
...
async def test_accept_requires_authentication(client):
...
async def test_accept_consumes_the_invite_so_it_cannot_be_used_twice(client):
...
async def test_accept_removes_the_pending_invite_row(client, db_session):
...
async def test_accept_is_a_no_op_if_the_link_holder_is_already_a_member(client):
...
async def test_accept_is_rate_limited(client):
