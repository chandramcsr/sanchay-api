...
async def test_full_reset_flow_changes_password(client, db_session):
    await _signup(client, password="oldpass123")
    user = await get_one(db_session, User, email="reset@example.com")
...
async def test_expired_reset_token_is_rejected(client, db_session):
