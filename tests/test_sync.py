async def _signup(client, email="sync@example.com", password="hunter2222"):
    r = await client.post("/api/v1/auth/signup", json={"email": email, "password": password, "display_name": "Sync Test"})
...
async def test_status_reports_no_backup_for_a_fresh_account(client):
    ...
    r = await client.get("/api/v1/sync/status", headers=_auth(token))
...
async def test_pull_returns_404_when_nothing_pushed_yet(client):
    token = await _signup(client)
    r = await client.get("/api/v1/sync/pull", headers=_auth(token))
    assert r.status_code == 404


async def test_first_push_creates_version_1(client):
    token = await _signup(client)
    r = await client.put("/api/v1/sync/push", headers=_auth(token), json={
...
async def test_push_with_stale_version_is_rejected_with_409(client):
    ...
    pulled = await client.get("/api/v1/sync/pull", headers=_auth(token))
...
async def test_sync_endpoints_require_authentication(client):
    ...
    assert (await client.put("/api/v1/sync/push", json={"ciphertext": "x", "encryption_meta": "y", "based_on_version": 0})).status_code == 401
...
async def test_sync_data_is_isolated_between_users(client):
    ...
    await client.put("/api/v1/sync/push", headers=_auth(tokenA), json={
        "ciphertext": "userA-secret-ledger", "encryption_meta": "m", "based_on_version": 0,
    })

    statusB = await client.get("/api/v1/sync/status", headers=_auth(tokenB))
    assert statusB.json()["exists"] is False

    pullB = await client.get("/api/v1/sync/pull", headers=_auth(tokenB))
