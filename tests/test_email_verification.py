...
async def _signup(client, email="verify-flow@example.com", password="hunter2222"):
    r = await client.post("/api/v1/auth/signup", json={"email": email, "password": password, "display_name": "Verify Flow"})
...
async def test_unverified_account_can_still_sign_in_and_use_the_app(client):
    ...
    r = await client.post("/api/v1/auth/login", json={"email": "verify-flow@example.com", "password": "hunter2222"})
...
async def test_verify_email_with_valid_token_marks_account_verified(client, db_session):
    ...
    r = await client.post("/api/v1/auth/verify-email", json={"token": raw_token})
    assert r.status_code == 200
...
async def test_verify_email_does_not_require_authentication(client, db_session):
    ...
    r = await client.post("/api/v1/auth/verify-email", json={"token": raw_token})  # no Authorization header
    assert r.status_code == 200
...
async def test_resend_verification_sends_a_fresh_token(client, db_session):
    ...
    r = await client.post("/api/v1/auth/resend-verification", headers=_auth(token))
    assert r.status_code == 200
