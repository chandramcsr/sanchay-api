...
async def create_group(
    ...
    for email in payload.member_emails:
        ...
        if user is None:
            # v1 scope: only existing Sanchay accounts can be added to
            # a group. Inviting someone who doesn't have an account
            # yet (triggering their signup) is a real, separate
            # feature, deliberately not built here — see backlog.
            raise HTTPException(status.HTTP_400_BAD_REQUEST, detail=f"No Sanchay account found for {email}")
...
async def get_group_detail(
    ...
    if group is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Group not found")
    members = await svc.get_group_members(db, group_id=group_id)
    return GroupOut(
        id=group.id,
        name=group.name,
        members=[GroupMemberOut(user_id=m.user_id, name=m.name_snapshot) for m in members],
...
async def edit_expense(
    ...
    expense = await svc.edit_shared_expense(
        db, expense_id=expense_id, edited_by=current_user.id, new_amount=new_amount, new_description=payload.description
    )
    splits = await svc.get_expense_splits(db, expense_id=expense_id)
    return _expense_to_out(expense, splits)
